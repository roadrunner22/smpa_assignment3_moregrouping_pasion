...
impeach %>%
  group_by(for_impeachment) %>%
    summarize(avg_graduationpct = mean(pct_bachelors))
...
impeach %>%
  group_by(for_impeachment) %>%
    summarize(avg_trumpvote = mean(trump_percent))
...
impeach %>%
  filter(for_impeachment == "YES", margin_flag_2018 == "5_points_or_less") %>%
   summarize(count = n())
...
# "Democrats are in the cities" is a phrase I commonly hear. I'm interested in seeing how democrats, who have a large rural population, voted for impeachment / and see how rural populations affect outcome. So if they voted for impeachment and have a large rural population.


impeach %>%
  group_by(for_impeachment == "YES", rural_pop_above20pct == "ABOVE") %>%
   summarize(count = n())
